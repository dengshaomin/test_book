...
```kotlin
var spannableString: SpannableString = SpannableString(tx.text)
    spannableString.setSpan(
        MyClickSpan(this), 2, spannableString.length,
        Spanned.SPAN_EXCLUSIVE_EXCLUSIVE
    )
    tx.text = spannableString
    tx.setMovementMethod(LinkMovementMethod.getInstance());//不设置 没有点击事件
    tx.setHighlightColor(Color.TRANSPARENT); //去除点击后的颜色为透明
```

{% tabs %}
{% tab title="ClickSpan" %}
```kotlin
internal class MyClickSpan(var context: Context) : ClickableSpan() {
