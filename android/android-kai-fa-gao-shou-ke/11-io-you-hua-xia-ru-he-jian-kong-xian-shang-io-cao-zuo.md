...
### I/O 跟踪[¶](https://blog.yorek.xyz/android/paid/master/io\_3/#io) <a href="#io" id="io"></a>
...
#### 1. Java Hook[¶](https://blog.yorek.xyz/android/paid/master/io\_3/#1-java-hook) <a href="#1-java-hook" id="1-java-hook"></a>
...
在[Libcore.java](http://androidxref.com/6.0.1\_r10/xref/libcore/luni/src/main/java/libcore/io/Libcore.java)中可以找到一个挺不错的 Hook 点，那就是[BlockGuardOs](http://androidxref.com/6.0.1\_r10/xref/libcore/luni/src/main/java/libcore/io/BlockGuardOs.java)这一个静态变量。如何可以快速找到合适的 Hook 点呢？一方面需要靠经验，但是耐心查看和分析源码是必不可少的工作。
...
#### 2. Native Hook[¶](https://blog.yorek.xyz/android/paid/master/io\_3/#2-native-hook) <a href="#2-native-hook" id="2-native-hook"></a>

如果 Java Hook 不能满足需求，我们自然就会考虑 Native Hook 方案。Profilo 使用到是 PLT Hook 方案，它的性能比GOT Hook要稍好一些，不过 [GOT Hook](https://github.com/Tencent/matrix/tree/master/matrix/matrix-android/matrix-android-commons/src/main/cpp/elf\_hook) 的兼容性会更好一些。
...
因为使用的是 GOT Hook，我们需要选择一些有调用上面几个方法的 library。微信 Matrix 中选择的是libjavacore.so、libopenjdkjvm.so、libopenjdkjvm.so，可以覆盖到所有的 Java 层的 I/O 调用，具体可以参考[io\_canary\_jni.cc](https://github.com/Tencent/matrix/blob/master/matrix/matrix-android/matrix-io-canary/src/main/cpp/io\_canary\_jni.cc#L161)。
...
#### 3. 监控内容[¶](https://blog.yorek.xyz/android/paid/master/io\_3/#3) <a href="#3" id="3"></a>
...
![io\_3\_1](https://blog.yorek.xyz/assets/images/android/master/io\_3\_1.png)

下面是一次 I/O 操作的基本信息，在主线程对一个大小为 600KB 的“test.db”文件。

![io\_3\_2](https://blog.yorek.xyz/assets/images/android/master/io\_3\_2.png)

使用了 4KB 的 Buffer，连续读取 150 次，一次性把整个文件读完，整体的耗时是 10ms。因为连读读写时间和打开文件的总时间相同，我们可以判断出这次 read() 操作是一气呵成的，中间没有间断。

![io\_3\_3](https://blog.yorek.xyz/assets/images/android/master/io\_3\_3.png)

因为 I/O 操作真的非常频繁，采集如此多的信息，对应用程序的性能会造成多大的影响呢？我们可以看看是否使用 Native Hook 的耗时数据。

![io\_3\_4](https://blog.yorek.xyz/assets/images/android/master/io\_3\_4.png)

你可以看到采用 Native Hook 的监控方法性能损耗基本可以忽略，这套方案可以用于线上。

### 线上监控[¶](https://blog.yorek.xyz/android/paid/master/io\_3/#\_1) <a href="#_1" id="_1"></a>
...
![io\_3\_5](https://blog.yorek.xyz/assets/images/android/master/io\_3\_5.png)

#### 1. 主线程 I/O[¶](https://blog.yorek.xyz/android/paid/master/io\_3/#1-io) <a href="#1-io" id="1-io"></a>
...
#### 2. 读写 Buffer 过小[¶](https://blog.yorek.xyz/android/paid/master/io\_3/#2-buffer) <a href="#2-buffer" id="2-buffer"></a>
...
那应该选用多大的 Buffer 呢？我们可以跟据文件保存所挂载的目录的 block size 来确认 Buffer 大小，数据库中的[pagesize](http://androidxref.com/6.0.1\_r10/xref/frameworks/base/core/java/android/database/sqlite/SQLiteGlobal.java#61)就是这样确定的。
...
![io\_3\_6](https://blog.yorek.xyz/assets/images/android/master/io\_3\_6.png)
...
![io\_3\_7](https://blog.yorek.xyz/assets/images/android/master/io\_3\_7.png)
...
#### 3. 重复读[¶](https://blog.yorek.xyz/android/paid/master/io\_3/#3\_1) <a href="#3_1" id="3_1"></a>
...
#### 4. 资源泄漏[¶](https://blog.yorek.xyz/android/paid/master/io\_3/#4) <a href="#4" id="4"></a>

在崩溃分析中，我说过有部分的 OOM 是由于文件句柄泄漏导致。资源泄漏是指打开资源包括文件、Cursor 等没有及时 close，从而引起泄露。这属于非常低级的编码错误，但却非常普遍存在。

如何有效的监控资源泄漏？这里我利用了 Android 框架中的 StrictMode，StrictMode 利用[CloseGuard.java](http://androidxref.com/8.1.0\_r33/xref/libcore/dalvik/src/main/java/dalvik/system/CloseGuard.java)类在很多系统代码已经预置了埋点。
...
### I/O 与启动优化[¶](https://blog.yorek.xyz/android/paid/master/io\_3/#io\_1) <a href="#io_1" id="io_1"></a>
...
![io\_3\_8](https://blog.yorek.xyz/assets/images/android/master/io\_3\_8.png)
...
### 总结[¶](https://blog.yorek.xyz/android/paid/master/io\_3/#\_2) <a href="#_2" id="_2"></a>
...
### 课后作业[¶](https://blog.yorek.xyz/android/paid/master/io\_3/#\_3) <a href="#_3" id="_3"></a>
