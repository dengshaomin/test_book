...
### Android 的存储基础[¶](https://blog.yorek.xyz/android/paid/master/storage\_1/#android) <a href="#android" id="android"></a>

在讲具体的存储方法之前，我们应该对 Android 系统存储相关的一些基础知识有所了解。

#### 1. Android 分区[¶](https://blog.yorek.xyz/android/paid/master/storage\_1/#1-android) <a href="#1-android" id="1-android"></a>
...
![storage\_1\_1](https://blog.yorek.xyz/assets/images/android/master/storage\_1\_1.png)

什么是分区呢？分区简单来说就是将设备中的存储划分为一些互不重叠的部分，每个部分都可以单独格式化，用作不同的目的。这样系统就可以灵活的针对单独分区做不同的操作，例如在系统还原（recovery）过程，我们不希望会影响到用户存储的数据。

![storage\_1\_2](https://blog.yorek.xyz/assets/images/android/master/storage\_1\_2.png)
...
#### 2. Android 存储安全[¶](https://blog.yorek.xyz/android/paid/master/storage\_1/#2-android) <a href="#2-android" id="2-android"></a>
...
### 常见的数据存储方法[¶](https://blog.yorek.xyz/android/paid/master/storage\_1/#\_1) <a href="#_1" id="_1"></a>
...
#### 1. 关键要素[¶](https://blog.yorek.xyz/android/paid/master/storage\_1/#1) <a href="#1" id="1"></a>

在选择数据存储方法时，我一般会想到下面这几点，我把它们总结给你。

![storage\_1\_3](https://blog.yorek.xyz/assets/images/android/master/storage\_1\_3.png)
...
#### 2. 存储选项[¶](https://blog.yorek.xyz/android/paid/master/storage\_1/#2) <a href="#2" id="2"></a>
...
[SharedPreferences](http://androidxref.com/9.0.0\_r3/xref/frameworks/base/core/java/android/content/SharedPreferences.java)是 Android 中比较常用的存储方法，它可以用来存储一些比较小的键值对集合。

虽然 SharedPreferences 使用非常简便，但也是我们诟病比较多的存储方法。它的性能问题比较多，我可以轻松地说出它的“七宗罪”。

* 跨进程不安全。由于没有使用跨进程的锁，就算使用[MODE\_MULTI\_PROCESS](https://developer.android.com/reference/android/content/Context)，SharedPreferences 在跨进程频繁读写有可能导致数据全部丢失。根据线上统计，SP 大约会有万分之一的损坏率。
...
![storage\_1\_4](https://blog.yorek.xyz/assets/images/android/master/storage\_1\_4.png)

你可以参考 MMKV 的[实现原理](https://github.com/Tencent/MMKV/wiki/design)和[性能测试报告](https://github.com/Tencent/MMKV/wiki/android\_benchmark\_cn)，里面有一些非常不错的思路。例如[利用文件锁保证跨进程的安全](https://github.com/Tencent/MMKV/wiki/android\_ipc)、使用 mmap 保证数据不会丢失、选用性能和存储空间更好的 Protocol Buffer 代替 XML、支持增量更新等。
...
* 启动性能\
  ...
  ![storage\_1\_5](https://blog.yorek.xyz/assets/images/android/master/storage\_1\_5.png)\
...
* 稳定性\
  ...
  ![storage\_1\_6](https://blog.yorek.xyz/assets/images/android/master/storage\_1\_6.png)\
...
![storage\_1\_7](https://blog.yorek.xyz/assets/images/android/master/storage\_1\_7.png)

总的来说，ContentProvider 这套方案实现相对比较笨重，适合传输大的数据。

### 总结[¶](https://blog.yorek.xyz/android/paid/master/storage\_1/#\_2) <a href="#_2" id="_2"></a>
