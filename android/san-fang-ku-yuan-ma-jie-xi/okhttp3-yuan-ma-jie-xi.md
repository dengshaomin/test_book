...
### 1. OkHttpClient以及Request的构造器[¶](https://blog.yorek.xyz/android/3rd-library/okhttp/#1-okhttpclientrequest) <a href="#1-okhttpclientrequest" id="1-okhttpclientrequest"></a>
...
### 2. Call & RealCall[¶](https://blog.yorek.xyz/android/3rd-library/okhttp/#2-call-realcall) <a href="#2-call-realcall" id="2-call-realcall"></a>
...
### 3. RealCall.execute[¶](https://blog.yorek.xyz/android/3rd-library/okhttp/#3-realcallexecute) <a href="#3-realcallexecute" id="3-realcallexecute"></a>
...
### 4. RealCall.enqueue[¶](https://blog.yorek.xyz/android/3rd-library/okhttp/#4-realcallenqueue) <a href="#4-realcallenqueue" id="4-realcallenqueue"></a>
...
关于线程池的更多知识，可以参考[Android中的线程池](https://blog.yorek.xyz/android/framework/Android%E7%BA%BF%E7%A8%8B%E4%B8%8E%E7%BA%BF%E7%A8%8B%E6%B1%A0/#3-android)
...
### 5. getResponseWithInterceptorChain[¶](https://blog.yorek.xyz/android/3rd-library/okhttp/#5-getresponsewithinterceptorchain) <a href="#5-getresponsewithinterceptorchain" id="5-getresponsewithinterceptorchain"></a>
...
7. Response的返回与Request相反，会从最后一个开始依次往前经过这些`Intercetor`

下图为OkHttp工作的大致流程，参考自[拆轮子系列：拆 OkHttp](https://blog.piasy.com/2016/07/11/Understand-OkHttp/index.html)

![OkHttp总体工作流程](https://blog.yorek.xyz/assets/images/android/okhttp\_overview.jpg)

接下来，我们看一下各个拦截器具体的代码。

### 6. RetryAndFollowUpInterceptor[¶](https://blog.yorek.xyz/android/3rd-library/okhttp/#6-retryandfollowupinterceptor) <a href="#6-retryandfollowupinterceptor" id="6-retryandfollowupinterceptor"></a>
...
### 7. BridgeInterceptor[¶](https://blog.yorek.xyz/android/3rd-library/okhttp/#7-bridgeinterceptor) <a href="#7-bridgeinterceptor" id="7-bridgeinterceptor"></a>
...
上面代码总体来说干了两件事： 1. 对原始的Request进行检查，设置`Content-Type`、`Content-Length`、`Transfer-Encoding`、`Host`、`Connection`、`Accept-Encoding`、`Cookie`、`User-Agent`这些header 2. 进行网络请求。若是gzip编码，则对响应进行Gzip处理；否则直接返回
...
### 8. CacheInterceptor[¶](https://blog.yorek.xyz/android/3rd-library/okhttp/#8-cacheinterceptor) <a href="#8-cacheinterceptor" id="8-cacheinterceptor"></a>

Comments

Serves requests from the cache and writes responses to the cache.

首先需要注意的是，OkHttp中的Cache策略采用的是`DiskLruCache`，关于`DiskLruCache`可以参考[DiskLruCache](https://blog.yorek.xyz/android/framework/Bitmap%E7%9A%84%E7%BC%93%E5%AD%98%E4%B8%8E%E5%8A%A0%E8%BD%BD/#22-disklrucache)。key的计算为：

```
ByteString.encodeUtf8(url.toString()).md5().hex()
```

Cache的一般策略如下图：![](https://blog.yorek.xyz/assets/images/android/cache.webp)Cache的一般策略
...
### 9. ConnectInterceptor[¶](https://blog.yorek.xyz/android/3rd-library/okhttp/#9-connectinterceptor) <a href="#9-connectinterceptor" id="9-connectinterceptor"></a>
...
也就是说，在建立连接时，如果是HTTP\_2，就会初始化一个`http2Connection`。
...
我们发现，这里根据`http2Connection`有没有初始化，也就是是不是HTTP\_2协议来创建对应的`HttpCodec`对象。

在拦截器的最后，调用`realChain.proceed(request, streamAllocation, httpCodec, connection)`将请求传递给下一个拦截器。

### 10. CallServerInterceptor[¶](https://blog.yorek.xyz/android/3rd-library/okhttp/#10-callserverinterceptor) <a href="#10-callserverinterceptor" id="10-callserverinterceptor"></a>
...
### 11. 小结[¶](https://blog.yorek.xyz/android/3rd-library/okhttp/#11) <a href="#11" id="11"></a>
...
下图为OkHttp工作的大致流程，参考自[拆轮子系列：拆 OkHttp](https://blog.piasy.com/2016/07/11/Understand-OkHttp/index.html)![](https://blog.yorek.xyz/assets/images/android/okhttp\_overview.jpg)OkHttp总体工作流程

### FAQ[¶](https://blog.yorek.xyz/android/3rd-library/okhttp/#faq) <a href="#faq" id="faq"></a>

[Retrofit/FAQ](https://blog.yorek.xyz/android/3rd-library/retrofit/#faq)

### 参考文献[¶](https://blog.yorek.xyz/android/3rd-library/okhttp/#\_1) <a href="#_1" id="_1"></a>
